...
export async function POST() {
  try {
    ...
    return NextResponse.json({
      message: `Seed completado. ${Object.keys(result.insertedIds).length} productos insertados.`,
      count: Object.keys(result.insertedIds).length,
    });
  } catch (err) {
    console.error("Seed error:", err);
    return NextResponse.json({ message: "Error al hacer seed", error: String(err) }, { status: 500 });
  }
}
